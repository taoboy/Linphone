...
public class SlidingDrawer extends ViewGroup {
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		if (mVertical) {
			...
			mContent.measure(MeasureSpec.makeMeasureSpec(widthSpecSize, MeasureSpec.EXACTLY), MeasureSpec.makeMeasureSpec(height, MeasureSpec.AT_MOST));
		} else {
	...
	protected void dispatchDraw(Canvas canvas) {
		...
		if (mTracking || mAnimating) {
			...
			if (cache != null) {
				if (isVertical) {
					if (mInvert) {
						canvas.drawBitmap(cache, 0, handle.getTop() - (getCustomBottom() - getTop()) + mHandleHeight, null);
					} else {
		...
		if (mTracking) {
			...
			case MotionEvent.ACTION_CANCEL: {
				...
				if (Math.abs(velocity) < mMaximumTapVelocity) {
					...
					if (mInvert) {
						c1 = (mExpanded && (getCustomBottom() - handleBottom) < mTapThreshold + mBottomOffset);
						...
						c2 = (!mExpanded && handleTop > mBottomOffset + getCustomBottom() - getTop() - mHandleHeight - mTapThreshold);
	...
	private void moveHandle(int position) {
		...
		if (mVertical) {
			if (position == EXPANDED_FULL_OPEN) {
				if (mInvert)
					handle.offsetTopAndBottom(mBottomOffset + getCustomBottom() - getTop() - mHandleHeight);
				else
				...
				if (mInvert) {
					...
					handle.offsetTopAndBottom(mBottomOffset + getCustomBottom() - getTop() - mHandleHeight - handle.getTop());
				}
				invalidate();
			} else {
				...
				} else if (deltaY > mBottomOffset + getCustomBottom() - getTop() - mHandleHeight - top) {
					deltaY = mBottomOffset + getCustomBottom() - getTop() - mHandleHeight - top;
				}
	...
	private void prepareContent() {
		...
		if (content.isLayoutRequested()) {
			...
			if (mVertical) {
				...
				content.measure(MeasureSpec.makeMeasureSpec(getRight() - getLeft(), MeasureSpec.EXACTLY), MeasureSpec.makeMeasureSpec(height, MeasureSpec.AT_MOST));
