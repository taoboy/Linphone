...
public class RemoteProvisioningActivity extends Activity implements LinphoneRemoteProvisioningListener {
		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				if (openUri != null) {
					...
					configUriParam = openUri.getEncodedSchemeSpecificPart().substring(2); // Removes
																							// the
																							// linphone-config://
					try {
				...
				if (configUriParam == null) {
					if (!LinphonePreferences.instance().isFirstRemoteProvisioning()) {
						...
						// Show this view for a few seconds then go to the
						// dialer
					...
					} // else we do nothing if there is no config uri parameter
						// and if user not allowed to leave this screen
				} else {
					if (getResources().getBoolean(R.bool.display_confirmation_popup_after_first_configuration) && !LinphonePreferences.instance().isFirstRemoteProvisioning()) {
						mHandler.post(new Runnable() {
							@Override
							public void run() {
	...
	private void displayDialogConfirmation() {
		new AlertDialog.Builder(RemoteProvisioningActivity.this).setTitle(getString(R.string.remote_provisioning_again_title)).setMessage(getString(R.string.remote_provisioning_again_message)).setPositiveButton(R.string.button_ok, new DialogInterface.OnClickListener() {
	...
	private void setRemoteProvisioningAddressAndRestart(String configUri) {
		...
		LinphonePreferences.instance().setContext(this); // Needed, else the
															// next call will
															// crash
	...
	private void goToLinphoneActivity() {
		if (LinphoneService.isReady()) {
			...
			// finish(); // To prevent the user to come back to this page using
			// back button
