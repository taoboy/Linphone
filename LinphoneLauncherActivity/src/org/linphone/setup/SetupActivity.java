...
public class SetupActivity extends FragmentActivity implements OnClickListener, LinphoneCoreListener {
	...
	public void saveCreatedAccount(String username, String password, String domain) {
		...
		if (isMainAccountLinphoneDotOrg && useLinphoneDotOrgCustomPorts) {
			...
			builder.setExpires("604800").setOutboundProxyEnabled(true).setAvpfEnabled(true).setAvpfRRInterval(3).setQualityReportingCollector("sip:voip-metrics@sip.linphone.org").setQualityReportingEnabled(true).setQualityReportingInterval(180).setRealm("sip.linphone.org");
		...
		try {
			builder.setEnabled(false); // We'll enable it after the echo
										// calibration
